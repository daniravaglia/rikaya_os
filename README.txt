...
init.c:
    File che contiene il main; qui vengono inizializzate le new areas per la gestione degli interrupt/trap
...
handler.c:
    Contiene tutte le funzioni per gestire sia le trap che gli interrupt.
    Nello specifico in questa fase gestisce solo l'interrupt relativo all'intervall timer e la trap relativa alle system call.
    In caso di altre eccezioni invocherà la funzione di PANIC. 

pcb.c:
    Qui sono contenuto tutte le funzioni relative ai pcb, implementate durante la fase 1. 
